...
{
    struct msg data;
    queue = xQueueCreate(6, sizeof(data)); 
    if (queue == 0)
    {
    ...
    data.id = 3;
    sprintf(data.mess, "Hello from Demo_Task4.0");
    xQueueSend(queue, &data, (TickType_t)0);  

    data.id = 2;
    sprintf(data.mess, "Hello from Demo_Task3.1");
    xQueueSend(queue, &data, (TickType_t)0);  

    data.id = 3;
    sprintf(data.mess, "Hello from Demo_Task4.1");
    xQueueSend(queue, &data, (TickType_t)0);  
     ...
     if( xQueueReceive(queue, &(rxBuffer), (TickType_t)5))
     {
        if (rxBuffer.id == 3) {
            ...
            vTaskDelay(1000/ portTICK_RATE_MS); 
             ...
             xQueueSendToFront(queue, &rxBuffer, (TickType_t)0);
             printf("Task4 miss read the data\n");
             vTaskDelay(1000/ portTICK_RATE_MS); 
         }
     }
     else {
...
void app_main(void)
{
   ...
   xTaskCreatePinnedToCore(Demo_Task4, "Demo_Task4", 4096, NULL,10, &myTaskHandle4, 1);
 }